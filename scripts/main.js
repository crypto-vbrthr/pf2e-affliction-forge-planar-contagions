import {
  PLANAR_CONTAGIONS_CONTENT_VERSION,
  PLANAR_CONTAGIONS_DEFINITIONS,
  PLANAR_CONTAGIONS_MODULE_ID
} from "./content.js";

const MODULE_ID = PLANAR_CONTAGIONS_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "planar-contagions";
const LIBRARY_ID = "planar-contagions.core";
const PACK_NAME = "affliction-forge-planar-contagions";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_PLANAR.Module.Library", "Planar Contagions"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: PLANAR_CONTAGIONS_CONTENT_VERSION,
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_PLANAR.Module.Title", "PF2E Affliction Forge: Planar Contagions"),
    moduleId: MODULE_ID,
    version: PLANAR_CONTAGIONS_CONTENT_VERSION,
    metadata: {
      themes: ["planar", "extraplanar", "affliction"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_PLANAR.Module.LibraryDescription", "Original planar afflictions with semantic Creature Forge tags."),
      metadata: {
        themes: ["planar", "extraplanar", "affliction"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: PLANAR_CONTAGIONS_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: PLANAR_CONTAGIONS_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_PLANAR.Module.SyncFailed", "Planar Contagions could not synchronize its library. See console.")
    );
  });
});
